...
func (pq *PlayerQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, player.UserTable, player.UserColumn),
		)
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withUser; query != nil {
		if err := pq.loadUser(ctx, query, nodes, nil,
			func(n *Player, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Player, init func(*Player), assign func(*Player, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Player)
	for i := range nodes {
		if nodes[i].user_players == nil {
			continue
		}
		fk := *nodes[i].user_players
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_players" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
