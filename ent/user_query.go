...
func (uq *UserQuery) QueryPlayers() *PlayerQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.PlayersTable, user.PlayersColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) loadPets(ctx context.Context, query *PetQuery, nodes []*User, init func(*User), assign func(*User, *Pet)) error {
	...
	for _, n := range neighbors {
		fk := n.master_name
		if fk == nil {
			return fmt.Errorf(`foreign-key "master_name" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "master_name" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*User, init func(*User), assign func(*User, *Player)) error {
	...
	for _, n := range neighbors {
		fk := n.user_players
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_players" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_players" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
