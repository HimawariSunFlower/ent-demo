...
func (pq *PetQuery) QueryMaster() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, pet.MasterTable, pet.MasterColumn),
		)
...
func (pq *PetQuery) Clone() *PetQuery {
	...
	return &PetQuery{
		...
		withMaster: pq.withMaster.Clone(),
...
// WithMaster tells the query-builder to eager-load the nodes that are connected to
// the "master" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PetQuery) WithMaster(opts ...func(*UserQuery)) *PetQuery {
	query := &UserQuery{config: pq.config}
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withMaster; query != nil {
		if err := pq.loadMaster(ctx, query, nodes, nil,
			func(n *Pet, e *User) { n.Edges.Master = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadMaster(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Pet)
	for i := range nodes {
		if nodes[i].master_name == nil {
			continue
		}
		fk := *nodes[i].master_name
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "master_name" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
